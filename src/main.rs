...
fn newton((x1, y1): (f64, f64), (x2, y2): (f64, f64), polinom: &[f64], h: u32) -> (u32, u32, Vec<u8>) {
...
fn main() -> Result<(), std::io::Error> {
    let matches = App::new("Фракталы Ньютона")
        ...
        .arg(Arg::with_name("polinom")
            ...
            .validator(|v| {
                for n in v.split(" ") {
                    if n != "" {
                        if let Err(_) = n.parse::<f64>() {
                            return Err(String::from("Многочлен должен состоять из чисел"))
                        }
                    }
                }
                Ok(())
    ...
    let mut polinom = matches.value_of("polinom").unwrap().split(" ").filter_map(|x| if x == "" {None} else {Some(x.trim().parse::<f64>().unwrap())}).collect::<Vec<_>>();
    polinom.reverse();
